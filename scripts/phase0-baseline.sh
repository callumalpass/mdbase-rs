...
if [[ -z "${MDBASE_CONNECT_COMMIT:-}" ]]; then
  if [[ -n "${MDBASE_CONNECT_DIR:-}" ]]; then
    export MDBASE_CONNECT_COMMIT="$(git -C "${MDBASE_CONNECT_DIR}" rev-parse HEAD)"
  else
    export MDBASE_CONNECT_COMMIT="not-observed"
  fi
fi
